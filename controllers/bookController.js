...
exports.book_delete_get = function(req, res) {
	async.parallel({
		...
		if (err) { return next(err); }
		//Successful, so render
		res.render('book_delete', { title: 'Delete Book', book: results.book, bookinstance_books: results.bookinstance_books } );
	});
};
...
exports.book_delete_post = function(req, res) {
	...
	async.parallel({
		...
		bookinstance_books: function(callback) {
			BookInstance.find({ 'book': req.body.bookid },'title').exec(callback);
		},
	}, function(err, results) {
		if (err) { return next(err); }
		//Success
		if (results.bookinstance_books>0) {
